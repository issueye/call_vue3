...
const currentStatus = computed(() => statusConfig[props.patient.status] || statusConfig[2])
...
<template>
  <div
    class="patient-item"
    :class="{ 'patient-item--active': active, 'patient-item--passed': patient.status === 4 }"
    @click="emit('click', patient)"
  >
    <div class="patient-item__header">
      <span class="patient-item__name">{{ patient.name }}</span>
      <BaseBadge :text="currentStatus.label" :class="currentStatus.class" />
    </div>

    <div class="patient-item__info">
      <span class="patient-item__meta">{{ genderLabel }} | {{ patient.age }}岁</span>
      <span class="patient-item__queue">排队号: {{ patient.queueNo }}</span>
    </div>
